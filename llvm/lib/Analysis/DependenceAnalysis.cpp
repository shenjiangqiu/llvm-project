...
void llvm::RefGroup::AddToGroup(llvm::Instruction *inst,bool IsLeader){
  //TODO
...
bool DependenceInfo::isSpatialReuse(Instruction* Src,Instruction* Dst){
  if (Src == Dst)
    return false;
  ...
  //WZL try to use GEP to find subscript
  ...
  if (UsefulGEP) { //since we have the infomation of pairs we can test if it's spatial reuse.
    ...
    for (GEPOperator::const_op_iterator SrcIdx = SrcGEP->idx_begin(),
      ...
      if(SrcIdx<SrcEnd-1){
        if(Pair[P].Src!=Pair[P].Dst){
          return false;
        }
      }
      unifySubscriptType(&Pair[P]);
    }
    return true;//that's spatial reuse!!
  }
  return false;
}
...
bool DependenceInfo::IsInSameGroup(Instruction *inst, RefGroup *group,
  ...
  for (auto group_inst : group->GetInstructions()) {
    ...
    if (d) { // temporal reuse
      ...
      auto t_level = d->getLevels();
      // errs()<<"inst1: "<<(inst->getFunction()->getName())<<", "<<*inst<<"\n";
      // errs()<<"inst2: "<<(group->GetLeader()->getFunction()->getName())<<",
      // "<<*(group->GetLeader())<<"\n";
      bool temporalReuse = true;
      for (auto i = 1u; i <= t_level; i++) {
        auto distance = d->getDistance(i);
        if (distance) {
          // errs()<<*distance<<" ";
          if (i != level) {
            if (!distance->isZero()) { // not level, must zero!!
              temporalReuse = false;
            }
          }
        }
...
std::vector<std::vector<RefGroup>> DependenceInfo::build_Groups(BasicBlock* block,AAResults& AA){

  //for each loop:
  static int loopNest=0;
  auto loopDeps = LI->getLoopDepth(block);
  std::vector<std::vector<RefGroup>> groups;
  for (int i = 1; i <= loopDeps; i++) {
    std::vector<RefGroup> level_groups;
    for (auto &&inst : *block) {
      if(!isLoadOrStore(&inst)){
        continue;
      }
      if (level_groups.empty()) {
        level_groups.push_back(RefGroup(&inst));
      } else {
        bool added = false;
    ...
    groups.push_back(level_groups);
  }
  return groups;
}


void DependenceInfo::processAllLoops(Loop *loop,ScalarEvolution &SE,AAResults& AA)  {
  auto child_loops = loop->getSubLoops();


  if (child_loops.size() == 0) { // the inner most loop

    for( auto blocks:loop->getBlocks()){
...
bool DependenceAnalysisWrapperPass::runOnFunction(Function &F) {
  ...
  for (auto &loop : LI) {
    info->processAllLoops(loop, SE,AA);
  }
...
DependenceInfo::depends(Instruction *Src, Instruction *Dst,
  ...
  if (UsefulGEP) {
    ...
    //WZL: Delinearize to find subscript pairs
